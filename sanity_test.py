    ...
    if passed:
      ...
      if not dec.headers(wtr, typ):
        print "CANNOT FORMAT test type=",typ
          ...
          for i, row in enumerate(debug_buffer):
            wtr.writerow(["t {}".format(i)] + [dec.bin2hex(b) for b in row[0].split('\t')])
            wtr.writerow(["r {}".format(i)] + [dec.bin2hex(b) for b in row[1].split('\t')])
...
p2sc_tests = [
  ...
  ("CPU basic test",
        TestCase(os.path.join(file_locations,'cpu-basictest.circ'),
                 os.path.join(file_locations,'reference_output/cpu-basictest.out')), "cpu"),
]
